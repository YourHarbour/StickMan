...
public class LevelManager implements Level{
    ...
    /**
     * Indicates if the level is reloaded after save
     */
     ...
     * @param recommendTime The recommend time for level
     * @param hp The hp of the hero now
     */
    public LevelManager(GameEngine model, String filename, double height, double width, double floorHeight, double heroX, String heroSize, List<Entity> entities, List<MovingEntity> movingEntities, List<Interactable> interactables, long recommendTime,int hp) {
        this.model = model;
    ...
    public void tick() {
        ...
        for (MovingEntity entity : this.movingEntities) {
            if(!entity.isActive() && !isReload){
                if(entity.getClass().getName().equals("stickman.entity.moving.enemy.Slime")){
                    this.model.getScoreText().killSlime();
                }
            }
        }

        for(Interactable interactable:interactables){
            if(!interactable.isActive() && !isReload){
                if(interactable.getClass().getName().equals("stickman.entity.still.Mushroom")){
                    this.model.getScoreText().touchMushroom();
                }
        ...
        if(isReload){
            for(Entity entity:entities){
                for(Entity entity1:this.model.getCareTaker().getMemento().getEntityList()){
                    if(!entity.getClass().getName().equals("stickman.entity.still.Platform") && !entity.getClass().getName().equals("stickman.entity.still.Flag")){
                        if(entity.getImagePath().equals(entity1.getImagePath())){
                            entity.setX(entity1.getXPos());
                            entity.setY(entity1.getYPos());
                        }
                    }
                }
            }
    ...
    @Override
    public void win() {
        LevelFacade.getNextLevel(this,this.model);
    }
    ...
    @Override
    public List<MovingEntity> getMoving() {
        return this.movingEntities;
    }

    @Override
    public StickMan getHero() {
        return (StickMan) this.hero;
    }
